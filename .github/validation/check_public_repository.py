...
PROJECT_DOCUMENTATION_NAMES = ("ProCGroups_pages",)
...
HOME_LIBRARY_LINK = (
    '<a href="https://github.com/n-yamaguchi-0729/YamaLean4Lib" '
...
def filesystem_inventory(root: Path) -> tuple[list[str], set[str]]:
    ...
    while pending:
        ...
        for entry in entries:
            if not relative_directory.parts and entry.name == ".git":
                continue
...
def tracked_inventory(root: Path) -> tuple[list[str], set[str]]:
    ...
    files = {relative for relative in decoded.split("\0") if relative}
    errors: list[str] = []
    casefolded: dict[str, str] = {}
...
def check_public_presentation(root: Path, errors: list[str]) -> None:
    ...
    for relative in ("homepage-en.html", "homepage-jp.html"):
        ...
        if text.count(HOME_LIBRARY_LINK) != 1:
            errors.append(
                f"{relative}: legacy YamaLean4Lib GitHub link is missing "
                "or duplicated"
            )
...
def check_repository(root: Path, *, filesystem: bool) -> list[str]:
    ...
    for documentation_name in PROJECT_DOCUMENTATION_NAMES:
        documentation = root / documentation_name
